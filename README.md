...
The **Glance** tab is embedded locally and provides a richer viewer for its supported formats. Its **VTK Tetra View** selector keeps both workflows available: **Geometry** uses the ITK reader and retains Surface, Surface with edges, Wireframe, and Points; **Volume** resamples legacy ASCII or big-endian binary linear tetrahedral grids to a regular scalar volume and uses Glance's native GPU ray caster. Reopen the VTK file after changing the selector. Other legacy `.vtk` files continue through the ITK geometry reader, and the sample catalog is hidden from the embedded interface.
